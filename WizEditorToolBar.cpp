...
#define WIZEDITOR_ACTION_BAIDU           QObject::tr("Use \"ruwictionary\" search")
#define WIZEDITOR_ACTION_RIFMUS           QObject::tr("Use \"rifmus\" search")
...
//            break;
    ...
    if (!m_editor->selectedText().isEmpty())
    {
        ...
        menu->addAction(WIZEDITOR_ACTION_RIFMUS, this, SLOT(on_editor_rifmus_triggered()));
    }
    //
    if (editing)
    {
        if (!hasPasteMenu)
        {
            ...
            if (!m_editor->selectedText().isEmpty())
            {
                menu->addAction(WIZEDITOR_ACTION_CUT, this, SLOT(on_editor_cut_triggered()));
            }
...
void WizEditorToolBar::on_editor_baidu_triggered()
{
    ...
    QUrl url("https://ru.wiktionary.org/wiki/" + m_editor->page()->selectedText());
    QDesktopServices::openUrl(url);
}

void WizEditorToolBar::on_editor_rifmus_triggered()
{
    WizAnalyzer::getAnalyzer().logAction("editorMenuSearchByRifmus");
    QUrl url("https://rifmus.net/rifma/" + m_editor->page()->selectedText());
    QDesktopServices::openUrl(url);
}
